@extends('layouts.app')

@section('content')
    <div class="max-w-4xl mx-auto px-4 py-8">

        <!-- Header -->
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-2xl font-bold text-slate-800 flex items-center gap-2">
                <i class="fas fa-edit text-[#06B6D4]"></i> Edit User
            </h3>

            <a href="{{ route('admin.users.index') }}"
                class="px-4 py-2 border border-slate-300 text-slate-700 rounded-lg hover:bg-slate-100 transition flex items-center gap-2">
                <i class="fas fa-arrow-left"></i> Kembali
            </a>
        </div>

        <!-- Card -->
        <div class="bg-white shadow-lg rounded-xl p-6 border border-slate-200">

            @if (session('success'))
                <div id="successAlert" class="mb-4 flex items-center p-4 text-white bg-green-500 rounded-lg shadow">
                    <i class="fas fa-check-circle mr-2"></i> {{ session('success') }}
                </div>
            @endif

            <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                @csrf
                @method('PUT')

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <!-- Nama -->
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">
                            <i class="fas fa-user mr-1 text-[#06B6D4]"></i> Nama Lengkap
                        </label>
                        <input type="text" name="name" value="{{ old('name', $user->name) }}" required
                            class="w-full p-3 rounded-lg border border-slate-300 focus:border-[#06B6D4] focus:ring-[#06B6D4] focus:ring-1 outline-none">
                        @error('name')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>

                    <!-- Email -->
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">
                            <i class="fas fa-envelope mr-1 text-[#06B6D4]"></i> Email
                        </label>
                        <input type="email" disabled value="{{ $user->email }}"
                            class="w-full p-3 rounded-lg bg-slate-100 border border-slate-200 text-slate-500 cursor-not-allowed">
                        <input type="hidden" name="email" value="{{ $user->email }}">
                    </div>
                </div>

                <!-- Password -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">
                            <i class="fas fa-lock mr-1 text-[#06B6D4]"></i> Password Baru
                        </label>
                        <input type="password" name="password" placeholder="Kosongkan jika tidak diubah"
                            class="w-full p-3 rounded-lg border border-slate-300 focus:border-[#06B6D4] focus:ring-[#06B6D4] focus:ring-1">
                        @error('password')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">
                            <i class="fas fa-lock mr-1 text-[#06B6D4]"></i> Konfirmasi Password
                        </label>
                        <input type="password" name="password_confirmation"
                            class="w-full p-3 rounded-lg border border-slate-300 focus:border-[#06B6D4] focus:ring-[#06B6D4] focus:ring-1">
                        @error('password_confirmation')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                </div>

                <!-- Role -->
                <div class="mt-6">
                    <label class="block text-sm font-semibold text-slate-700 mb-1">
                        <i class="fas fa-user-tag mr-1 text-[#06B6D4]"></i> Role
                    </label>
                    <div class="w-full p-3 rounded-lg bg-slate-100 border border-slate-200 text-slate-600">
                        {{ ucfirst($user->role) }}
                    </div>
                    <input type="hidden" name="role" value="{{ $user->role }}">
                </div>

                <!-- Informasi User -->
                <div class="mt-6">
                    <h6 class="text-base font-semibold text-slate-700 flex items-center gap-2 mb-2">
                        <i class="fas fa-info-circle text-[#06B6D4]"></i> Informasi User
                    </h6>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                            <p class="text-slate-500">Email</p>
                            <p class="font-semibold">{{ $user->email }}</p>
                        </div>
                        <div>
                            <p class="text-slate-500">Role</p>
                            <p>{{ ucfirst($user->role) }}</p>
                        </div>
                        <div>
                            <p class="text-slate-500">Dibuat</p>
                            <p class="font-semibold">{{ $user->created_at->format('d/m/Y H:i') }}</p>
                        </div>
                        <div>
                            <p class="text-slate-500">Diperbarui</p>
                            <p class="font-semibold">{{ $user->updated_at->format('d/m/Y H:i') }}</p>
                        </div>
                    </div>
                </div>

                <!-- Buttons -->
                <div class="mt-8 flex justify-end gap-3">
                    <a href="{{ route('admin.users.index') }}"
                        class="px-4 py-2 border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-100 transition flex items-center gap-2">
                        <i class="fas fa-times"></i> Batal
                    </a>

                    <button type="submit"
                        class="px-5 py-3 bg-[#06B6D4] hover:bg-[#0891B2] text-white rounded-lg transition flex items-center gap-2">
                        <i class="fas fa-save"></i> Update User
                    </button>
                </div>
            </form>
        </div>
    </div>

    @if (session('success'))
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                setTimeout(() => {
                    const alert = document.getElementById('successAlert');
                    if (alert) {
                        alert.style.opacity = '0';
                        setTimeout(() => alert.remove(), 500);
                    }
                }, 5000);
            });
        </script>
    @endif
@endsection
